...
USER_PERMISSIONS_FEATURES_MCP_REMOTE_CUSTOM = (
    os.environ.get("USER_PERMISSIONS_FEATURES_MCP_REMOTE_CUSTOM", "True").lower()
    == "true"
)

USER_PERMISSIONS_FEATURES_MCP_STATIC_SECRETS = (
    os.environ.get("USER_PERMISSIONS_FEATURES_MCP_STATIC_SECRETS", "True").lower()
    == "true"
)

USER_PERMISSIONS_FEATURES_MCP_STDIO_TEMPLATES = (
    os.environ.get("USER_PERMISSIONS_FEATURES_MCP_STDIO_TEMPLATES", "True").lower()
    == "true"
)

USER_PERMISSIONS_FEATURES_MCP_STDIO_CUSTOM = (
    os.environ.get("USER_PERMISSIONS_FEATURES_MCP_STDIO_CUSTOM", "False").lower()
    == "true"
)
...
USER_PERMISSIONS_FEATURES_SUBAGENTS = (
    os.environ.get("USER_PERMISSIONS_FEATURES_SUBAGENTS", "True").lower() == "true"
)
...
DEFAULT_USER_PERMISSIONS = {
    ...
    "features": {
        ...
        "mcp_remote_custom": USER_PERMISSIONS_FEATURES_MCP_REMOTE_CUSTOM,
        "mcp_static_secrets": USER_PERMISSIONS_FEATURES_MCP_STATIC_SECRETS,
        "mcp_stdio_templates": USER_PERMISSIONS_FEATURES_MCP_STDIO_TEMPLATES,
        "mcp_stdio_custom": USER_PERMISSIONS_FEATURES_MCP_STDIO_CUSTOM,
...
Analyze the chat history to determine the necessity of generating a search query, in the given language. By default, **generate exactly 1 broad and relevant search query** unless it is absolutely certain that no additional information is required. The aim is to retrieve comprehensive, updated, and valuable information even with minimal uncertainty. If no search is unequivocally needed, return an empty list.

### Guidelines:
- Respond **EXCLUSIVELY** with a JSON object. Any form of extra commentary, explanation, or additional text is strictly prohibited.
- When generating a search query, respond in the format: { "queries": ["query1"] }, ensuring the query is distinct, concise, and relevant to the topic.
- Generate **ONLY ONE** search query - do not generate multiple queries.
- If and only if it is entirely certain that no useful results can be retrieved by a search, return: { "queries": [] }.
- Err on the side of suggesting a search query if there is **any chance** it might provide useful or updated information.
- Be concise and focused on composing a high-quality search query, avoiding unnecessary elaboration, commentary, or assumptions.
- Today's date is: {{CURRENT_DATE}}.
- Always prioritize providing an actionable and broad query that maximizes informational coverage.
...
DATA_VIZ_AUTO_REPAIR_ENABLED = PersistentConfig(
    ...
    "data_viz.auto_repair.enabled",
    os.getenv("DATA_VIZ_AUTO_REPAIR_ENABLED", "True").lower() == "true",
)

DATA_VIZ_AUTO_REPAIR_MAX_ATTEMPTS = PersistentConfig(
    "DATA_VIZ_AUTO_REPAIR_MAX_ATTEMPTS",
    "data_viz.auto_repair.max_attempts",
    int(os.getenv("DATA_VIZ_AUTO_REPAIR_MAX_ATTEMPTS", "3")),
)

DATA_VIZ_AUTO_REPAIR_MODEL = PersistentConfig(
    "DATA_VIZ_AUTO_REPAIR_MODEL",
    "data_viz.auto_repair.model",
