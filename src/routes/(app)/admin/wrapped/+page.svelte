<script lang="ts">
	...
	let rangePreset: RangePreset = $state('year');
	...
	let userUsage: UserUsage[] = $state([]);
	let subagents: SubagentAnalytics | null = $state(null);
	let spend: TotalSpend | null = $state(null);
	let spendTrend: DailySpendPoint[] = $state([]);
	let topChatsByCost: TopChat[] = $state([]);

	let search = $state('');
	let sortKey: 'tokens' | 'cache' | 'messages' | 'days' | 'avg' | 'cost' = $state('tokens');
	...
	const heatColors = [
		...
		'bg-kraft/60 dark:bg-kraft/40',
		'bg-book-cloth/60 dark:bg-book-cloth/50',
	...
	async function loadAll() {
		...
		try {
			...
			// the reactive guard can't re-fire loadAll() in a tight loop.
			if (seq === loadSeq) {
				loadedKey = key;
				loading = false;
			}
	...
	function modelName(
		model: { model_id?: string | null; model_name?: string | null } | string | null | undefined
	) {
		// Prefer the admin-configured pretty name (e.g. "Gemini 3.1 Pro C"); fall
		// back to the bare model id's last path segment.
		const name = typeof model === 'string' ? null : model?.model_name;
		if (name) return name;
	...
	let rangeKey = $derived(
		`${rangePreset}:${rangePreset === 'custom' ? `${customStart}-${customEnd}` : ''}:${selectedYear}`
	);
	$effect(() => {
	...
	let days = $derived(topDays(daily));
	let months = $derived(monthTotals(daily));
	let weekdays = $derived(weekdayTotals(daily));
	let maxMonth = $derived(Math.max(...months.map((m) => m.tokens), 0));
	let maxWeekday = $derived(Math.max(...weekdays.map((d) => d.tokens), 0));
	let totalTokens = $derived(wrapped?.total_tokens ?? 0);
	let totalCache = $derived(wrapped?.total_cache_read_tokens ?? 0);
	let totalInput = $derived(userUsage.reduce((sum, u) => sum + u.total_input_tokens, 0));
	let totalOutput = $derived(userUsage.reduce((sum, u) => sum + u.total_output_tokens, 0));
	let activeUsers = $derived(wrapped?.total_users_active ?? 0);
	let totalMessages = $derived(wrapped?.total_messages ?? 0);
	let cacheRate = $derived(pct(totalCache, totalInput));
	let avgPerUser = $derived(activeUsers > 0 ? Math.round(totalTokens / activeUsers) : 0);
	let avgPerMessage = $derived(totalMessages > 0 ? Math.round(totalTokens / totalMessages) : 0);
	let topUser = $derived(userUsage[0]);
	let topUserShare = $derived(topUser ? pct(topUser.total_tokens, totalTokens) : 0);
	...
	let selectedUser = $derived(
		userUsage.find((u) => u.user_id === selectedUserId) ?? matchingUsers[0] ?? null
	);
	...
	let subagentStatuses = $derived(
		Object.entries(subagents?.status_counts ?? {}).sort((a, b) => b[1] - a[1])
	);
	let totalCost = $derived(spend?.total_cost ?? 0);
	let maxTrendCost = $derived(Math.max(...spendTrend.map((p) => p.cost), 0));
...
<svelte:head>
	<title>Admin {selectedYear} Analytics | {$WEBUI_NAME}</title>
</svelte:head>
	...
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		<header class="mb-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
			<div>
				...
				<p class="mt-3 max-w-2xl text-sm leading-6 text-gray-600 dark:text-gray-400">
					Global usage, cache efficiency, model mix, active-user distribution, and per-user
					drilldown.
				</p>
			</div>
			...
			<div class="flex flex-wrap items-center gap-3">
				<div class="flex items-center gap-2">
					<label for="range-select" class="text-sm font-medium text-gray-500 dark:text-gray-400"
						>{$i18n.t('Range')}</label
					>
					<select
						id="range-select"
				...
				{:else if rangePreset === 'custom'}
					<input
						...
						class="rounded-lg border-hairline border-gray-200 bg-white px-3 py-2 text-sm shadow-sm outline-none transition-colors duration-200 ease-paper focus:border-book-cloth dark:border-gray-800 dark:bg-gray-900"
					/>
				...
				<button
					class="rounded-lg border-hairline border-book-cloth/25 bg-book-cloth/15 px-4 py-2 text-sm font-semibold text-book-cloth transition-colors duration-200 ease-paper hover:bg-book-cloth/25 dark:border-book-cloth/30 dark:bg-book-cloth/15 dark:text-kraft dark:hover:bg-book-cloth/25"
					onclick={() => goto('/admin/wrapped/cache')}
					title={$i18n.t('Cache lifetime, hit rates, and savings')}
				>
					{$i18n.t('Cache intelligence')}
				</button>
				<button
					class="rounded-lg border-hairline border-gray-200 bg-white px-4 py-2 text-sm font-semibold shadow-sm transition-colors duration-200 ease-paper hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:hover:bg-gray-800"
					onclick={() => goto('/admin/settings/pricing')}
		...
		{:else if wrapped}
			<section class="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
				<div
					class="rounded-2xl border-hairline border-success/25 bg-success/10 p-6 shadow-sm dark:border-success/30 dark:bg-success/15"
				>
					...
					<div class="mt-4 text-xs text-success/80 dark:text-success-dark/80">
						{$i18n.t('embedded')}
						{formatCost(spend?.embedded_cost ?? 0)} · {$i18n.t('rate-card')}
						{formatCost(spend?.rate_card_cost ?? 0)}
					</div>
					{#if (spend?.unpriced_model_count ?? 0) > 0}
						<button
							class="mt-2 text-xs font-medium text-warning underline-offset-2 hover:underline dark:text-warning-dark"
							onclick={() => goto('/admin/settings/pricing')}
						>
							{spend?.unpriced_model_count}
							{$i18n.t('models unpriced — map them')}
						</button>
					{/if}
				</div>
				<div
				...
				>
					...
					<div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
						{fullNumber(totalTokens)} total tokens
					</div>
				</div>
				<div
					class="rounded-2xl border-hairline border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900"
				>
					...
					<div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
						Avg/user: {formatTokenCount(avgPerUser)}
					</div>
				</div>
				<div
					class="rounded-2xl border-hairline border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900"
				>
					...
					<div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
						{fmtPct(cacheRate)} of prompt tokens cached
					</div>
				</div>
				<div
					class="rounded-2xl border-hairline border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900"
				>
					...
					<div
						class="mt-3 text-4xl font-semibold tracking-tight text-success dark:text-success-dark"
					>
						{formatTokenCount(totalMessages)}
					</div>
					<div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
						Avg/message: {formatTokenCount(avgPerMessage)}
					</div>
				</div>
			</section>
					...
					<div class="mb-5 flex items-center justify-between gap-4">
						<div>
							...
							<p class="text-sm text-gray-500 dark:text-gray-400">
								Prompt, output, and cache-reuse volume across all users.
							</p>
						</div>
						<div class="text-right text-sm text-gray-500 dark:text-gray-400">
							Top user share: {fmtPct(topUserShare)}
						</div>
					</div>
					<div class="space-y-5">
						<div>
							<div class="mb-2 flex justify-between text-sm">
								<span class="font-medium">Prompt tokens</span><span
									class="text-gray-500 dark:text-gray-400">{formatTokenCount(totalInput)}</span
								>
							</div>
						...
						<div>
							<div class="mb-2 flex justify-between text-sm">
								<span class="font-medium">Completion tokens</span><span
									class="text-gray-500 dark:text-gray-400">{formatTokenCount(totalOutput)}</span
								>
							</div>
							<div class="h-3 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
								<div
						...
						<div>
							<div class="mb-2 flex justify-between text-sm">
								<span class="font-medium">Cached prompt reads</span><span
									class="text-gray-500 dark:text-gray-400">{formatTokenCount(totalCache)}</span
								>
							</div>
							<div class="h-3 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
								<div class="h-full rounded-full bg-kraft" style="width: {cacheRate}%"></div>
				...
				>
					...
					<div class="mt-5 space-y-3">
						{#each roleBreakdown as role}
							<div
								class="flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3 dark:bg-gray-950/60"
							>
						...
						<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
							...
							<div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
								{formatTokenCount(topUser?.total_tokens ?? 0)} tokens
							</div>
						</div>
					</div>
				</div>
					...
					>
						<div class="mb-5 flex items-start justify-between gap-4">
							<div>
								...
								<p class="text-sm text-gray-500 dark:text-gray-400">
									Hidden worker chats folded into visible parent conversations.
								</p>
							</div>
							<span
								class="rounded-full bg-book-cloth/15 px-3 py-1 text-xs font-semibold text-book-cloth dark:bg-book-cloth/20 dark:text-kraft"
								>{subagents.token_share_percent.toFixed(1)}% of tokens</span
							>
						</div>

						<div class="grid grid-cols-2 gap-3">
							<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
								<div class="text-xs text-gray-500 dark:text-gray-400">Subagents spawned</div>
								<div class="mt-2 text-2xl font-semibold">
									{subagents.total_subagent_chats.toLocaleString()}
								</div>
							</div>
							<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
								<div class="text-xs text-gray-500 dark:text-gray-400">Parent chats</div>
								<div class="mt-2 text-2xl font-semibold">
									{subagents.parent_chat_count.toLocaleString()}
								</div>
							</div>
							<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
								<div class="text-xs text-gray-500 dark:text-gray-400">Model calls</div>
								<div class="mt-2 text-2xl font-semibold">
									{subagents.request_count.toLocaleString()}
								</div>
							</div>
							<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
								<div class="text-xs text-gray-500 dark:text-gray-400">Avg/subagent</div>
								<div class="mt-2 text-2xl font-semibold">
									{formatTokenCount(subagents.avg_tokens_per_subagent)}
								</div>
							</div>
						</div>
						...
						<div class="mt-5 space-y-3 text-sm">
							<div
								class="flex justify-between border-b-hairline border-gray-100 pb-2 dark:border-gray-800"
							>
								<span class="text-gray-500 dark:text-gray-400">Total tokens</span><span
									>{formatTokenCount(subagents.total_tokens)}</span
								>
							</div>
							<div
								class="flex justify-between border-b-hairline border-gray-100 pb-2 dark:border-gray-800"
							>
								<span class="text-gray-500 dark:text-gray-400">Prompt / output</span><span
									>{formatTokenCount(subagents.total_input_tokens)} / {formatTokenCount(
										subagents.total_output_tokens
									)}</span
								>
							</div>
							<div
								class="flex justify-between border-b-hairline border-gray-100 pb-2 dark:border-gray-800"
							>
								<span class="text-gray-500 dark:text-gray-400">Cache reads</span><span
									>{formatTokenCount(subagents.total_cache_read_tokens)}</span
								>
							</div>
							<div class="flex justify-between">
								<span class="text-gray-500 dark:text-gray-400">Avg requests/subagent</span><span
									>{subagents.avg_requests_per_subagent.toFixed(1)}</span
								>
							</div>
						</div>
							...
							<div class="mt-5">
								...
								<div class="flex flex-wrap gap-2">
									{#each subagentStatuses as [status, count]}
										<span
											class="rounded-full bg-gray-100 px-3 py-1 text-xs font-medium capitalize dark:bg-gray-800"
											>{status}: {count}</span
										>
									{/each}
								</div>
								...
								>
									<div class="flex items-start justify-between gap-4">
										<div class="min-w-0">
											...
											<div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
												{chat.subagent_count} subagents · {chat.request_count} calls · {formatTokenCount(
					...
					>
						...
						<div class="mt-5 space-y-3">
							{#each subagents.top_subagents.slice(0, 10) as sa}
								<a
									class="block rounded-2xl p-3 transition hover:bg-gray-50 dark:hover:bg-gray-950/60"
									href={`/c/${sa.subagent_chat_id}`}
									target="_blank"
									rel="noopener noreferrer"
								>
									<div class="flex items-start justify-between gap-4">
										<div class="min-w-0">
											<div class="truncate text-sm font-medium">{sa.title}</div>
											<div class="mt-1 truncate text-xs text-gray-500 dark:text-gray-400">
												parent: {sa.parent_title} · {sa.request_count} calls · {sa.source_type}
											</div>
										</div>
										<div class="shrink-0 text-sm font-semibold">
											{formatTokenCount(sa.total_tokens)}
										</div>
									</div>
								</a>
							{/each}
							...
							{#each subagents.top_models.slice(0, 8) as model}
								<div>
									<div class="mb-1 flex items-center justify-between gap-3 text-sm">
										<span class="truncate font-medium">{modelName(model)}</span><span
											class="text-gray-500 dark:text-gray-400">{model.percentage.toFixed(1)}%</span
										>
									</div>
			...
			>
				<div class="mb-5 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
					<div>
						...
						<p class="text-sm text-gray-500 dark:text-gray-400">
							Busiest day: {wrapped.busiest_day
								? `${shortDate(wrapped.busiest_day.date)} · ${formatTokenCount(wrapped.busiest_day.tokens)}`
								: '—'}
						</p>
					</div>
					<div class="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
						<span>Less</span>{#each heatColors as color}<span class="h-3 w-3 rounded-sm {color}"
							></span>{/each}<span>More</span>
					</div>
				</div>
							...
							<div class="flex items-center gap-3">
								...
								<div class="min-w-0 flex-1">
									...
									<div class="mt-1 h-1.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
										<div
											class="h-full rounded-full bg-book-cloth"
											style="width: {pct(day.tokens, days[0]?.tokens ?? 0)}%"
										></div>
									</div>
								</div>
				...
				>
					...
					<div class="mt-5 space-y-2">
						{#each months as month}
							<div class="grid grid-cols-[2.5rem_1fr_4rem] items-center gap-3 text-sm">
								<span class="text-gray-500 dark:text-gray-400">{month.month}</span>
								<div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
									<div
										class="h-full rounded-full bg-kraft"
										style="width: {pct(month.tokens, maxMonth)}%"
									></div>
								</div>
								<span class="text-right text-xs text-gray-500 dark:text-gray-400"
									>{formatTokenCount(month.tokens)}</span
								>
							</div>
						{/each}
					</div>
						...
						{#each weekdays as day}
							<div class="grid grid-cols-[2.5rem_1fr_4rem] items-center gap-3 text-sm">
								...
								<div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
									<div
										class="h-full rounded-full bg-book-cloth"
										style="width: {pct(day.tokens, maxWeekday)}%"
									></div>
								</div>
								<span class="text-right text-xs text-gray-500 dark:text-gray-400"
									>{formatTokenCount(day.tokens)}</span
								>
							</div>
						{/each}
					</div>
				...
				>
					<div class="mb-5 flex items-center justify-between gap-4">
						<div>
							...
							<p class="text-sm text-gray-500 dark:text-gray-400">
								{$i18n.t('Daily spend')} · {rangeLabel}
							</p>
						</div>
						<div class="text-right">
							<div class="text-2xl font-semibold text-success dark:text-success-dark">
								{formatCost(totalCost)}
							</div>
					...
					{#if spendTrend.length > 0 && maxTrendCost > 0}
						<div class="overflow-x-auto scrollbar-none">
							<div class="flex h-40 items-end gap-px min-w-full">
								{#each spendTrend as p}
									<div
										class="flex-1 min-w-[3px] rounded-t bg-gradient-to-t from-success to-success-dark transition hover:opacity-80"
							...
							>
								<span class="flex min-w-0 items-center gap-2">
									...
									<span class="truncate text-sm font-medium"
										>{chat.title || $i18n.t('Untitled')}</span
									>
								</span>
								<span class="shrink-0 text-sm font-semibold text-success dark:text-success-dark"
									>{formatCost(chat.cost)}</span
								>
							</button>
						{:else}
							<div class="py-8 text-center text-sm text-gray-400">
								{$i18n.t('No chats in range')}
							</div>
						{/each}
					</div>
				</div>
								...
								<div class="mb-1 flex items-center justify-between gap-3 text-sm">
									...
									<span class="flex items-center gap-2">
										{#if (model.unpriced_tokens ?? 0) > 0}
											<span
												class="rounded-full bg-warning/15 px-2 py-0.5 text-[10px] font-semibold text-warning dark:bg-warning/20 dark:text-warning-dark"
												>unmapped</span
											>
										{:else}
											<span class="font-semibold text-success dark:text-success-dark"
												>{formatCost(model.cost)}</span
											>
										{/if}
					...
					<div class="mt-5 grid gap-5 md:grid-cols-2">
							...
							{#each cacheUsers as u}
								<div>
									<div class="mb-1 flex justify-between gap-3 text-sm">
										<span class="truncate font-medium">{userLabel(u)}</span><span
											class="text-gray-500 dark:text-gray-400"
											>{formatTokenCount(u.total_cache_read_tokens)}</span
										>
									</div>
									<div class="h-1.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
										<div
											...
											style="width: {pct(
												...
												cacheUsers[0]?.total_cache_read_tokens ?? 0
											)}%"
										></div>
									</div>
								</div>
						...
						<div class="space-y-3">
							...
							{#each cacheModels as m}
								<div>
									<div class="mb-1 flex justify-between gap-3 text-sm">
										<span class="truncate font-medium">{modelName(m)}</span><span
											class="text-gray-500 dark:text-gray-400"
											>{formatTokenCount(m.total_cache_read_tokens)} · {fmtPct(cachePct(m))}</span
										>
									</div>
									<div class="h-1.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
										<div
			...
			<section class="grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(360px,0.9fr)]">
				...
				>
					<div class="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
						<div>
							<h2 class="text-xl font-semibold">Per-user analytics</h2>
							<p class="text-sm text-gray-500 dark:text-gray-400">
								Search, sort, and click a user for details.
							</p>
						</div>
						<div class="flex flex-col gap-2 sm:flex-row">
							<input
								...
								class="rounded-lg border-hairline border-gray-200 bg-white px-3 py-2 text-sm outline-none transition-colors duration-200 ease-paper focus:border-book-cloth focus:ring-4 focus:ring-book-cloth/10 dark:border-gray-800 dark:bg-gray-950"
							/>
							<select
								bind:value={sortKey}
								class="rounded-lg border-hairline border-gray-200 bg-white px-3 py-2 text-sm outline-none transition-colors duration-200 ease-paper focus:border-book-cloth focus:ring-4 focus:ring-book-cloth/10 dark:border-gray-800 dark:bg-gray-950"
					...
					<div class="overflow-x-auto">
						<table class="w-full min-w-[820px] text-left text-sm">
							<thead class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400"
								><tr
									><th class="py-3 pr-3">User</th><th class="px-3 py-3 text-right">Cost</th><th
										class="px-3 py-3 text-right">Tokens</th
									><th class="px-3 py-3 text-right">Cache</th><th class="px-3 py-3 text-right"
										>Messages</th
									><th class="px-3 py-3 text-right">Chats</th><th class="px-3 py-3 text-right"
										>Days</th
									></tr
								></thead
							>
							<tbody class="divide-y divide-gray-100 dark:divide-gray-800">
								{#each matchingUsers.slice(0, 100) as u}
									<tr
										class="cursor-pointer transition hover:bg-gray-50 dark:hover:bg-gray-950/70 {selectedUser?.user_id ===
										u.user_id
											? 'bg-book-cloth/10 dark:bg-book-cloth/15'
											: ''}"
										onclick={() => (selectedUserId = u.user_id)}
									>
										<td class="py-3 pr-3"
											><div class="font-medium">{userLabel(u)}</div>
											<div class="text-xs text-gray-500 dark:text-gray-400">
												{u.email || shortUserId(u.user_id)}
											</div></td
										>
										<td
											class="px-3 py-3 text-right font-semibold text-success dark:text-success-dark"
											>{formatCost(u.cost ?? 0)}</td
										>
										<td class="px-3 py-3 text-right font-medium"
											>{formatTokenCount(u.total_tokens)}</td
										>
										<td class="px-3 py-3 text-right text-book-cloth dark:text-kraft"
											>{formatTokenCount(u.total_cache_read_tokens)}</td
										>
					...
					{#if selectedUser}
						<div class="mt-5">
							...
							<div class="mt-5 grid grid-cols-2 gap-3">
								<div class="rounded-2xl bg-success/10 p-4 dark:bg-success/15">
									<div class="text-xs text-success dark:text-success-dark">Spend</div>
									<div class="mt-2 text-2xl font-semibold text-success dark:text-success-dark">
										{formatCost(selectedUser.cost ?? 0)}
									</div>
								</div>
								<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
									<div class="text-xs text-gray-500 dark:text-gray-400">Tokens</div>
									<div class="mt-2 text-2xl font-semibold">
										{formatTokenCount(selectedUser.total_tokens)}
									</div>
								</div>
								<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
									<div class="text-xs text-gray-500 dark:text-gray-400">Cache</div>
									<div class="mt-2 text-2xl font-semibold text-book-cloth dark:text-kraft">
										{formatTokenCount(selectedUser.total_cache_read_tokens)}
									</div>
								</div>
								<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60">
									<div class="text-xs text-gray-500 dark:text-gray-400">Cache rate</div>
									<div class="mt-2 text-2xl font-semibold">
										{selectedUser.cache_read_rate.toFixed(1)}%
									</div>
								</div>
							</div>
							<div class="mt-5 space-y-3 text-sm">
								<div
									class="flex justify-between border-b-hairline border-gray-100 pb-2 dark:border-gray-800"
								>
									<span class="text-gray-500 dark:text-gray-400">Prompt</span><span
										>{formatTokenCount(selectedUser.total_input_tokens)}</span
									>
								</div>
								<div
									class="flex justify-between border-b-hairline border-gray-100 pb-2 dark:border-gray-800"
								>
									<span class="text-gray-500 dark:text-gray-400">Output</span><span
										>{formatTokenCount(selectedUser.total_output_tokens)}</span
									>
								</div>
								<div
									class="flex justify-between border-b-hairline border-gray-100 pb-2 dark:border-gray-800"
								>
									<span class="text-gray-500 dark:text-gray-400">Active days</span><span
										>{selectedUser.days_active}</span
									>
								</div>
								<div class="flex justify-between">
									<span class="text-gray-500 dark:text-gray-400">Role</span><span class="capitalize"
										>{selectedUser.role || 'unknown'}</span
									>
								</div>
							</div>
						</div>
