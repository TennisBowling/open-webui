<script lang="ts">
	...
	let search = '';
	let sortKey: 'tokens' | 'cache' | 'messages' | 'days' | 'avg' = 'tokens';
	...
	async function loadAll() {
		...
		try {
			...
			const [wrappedData, heatmapData, modelData, userData, subagentData] = await Promise.all([
				getGlobalWrapped(token, selectedYear),
				getGlobalHeatmap(token, selectedYear),
				getGlobalModelUsage(token, 25, selectedYear),
				getGlobalUserUsage(token, selectedYear, 500),
					...
					{#if selectedUser}
						<div class="mt-5">
							...
							<div class="mt-5 grid grid-cols-2 gap-3">
								...
								<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950/60"><div class="text-xs text-gray-500 dark:text-gray-400">Avg/message</div><div class="mt-2 text-2xl font-semibold">{formatTokenCount(selectedUser.avg_tokens_per_message)}</div></div>
