...
<Collapsible grow={true} className="w-full" buttonClassName="w-full" bind:open>
	<div class="flex items-center gap-2 text-gray-500 transition">
		{@render children?.()}
		<ChevronDown
			strokeWidth="2.5"
			className="size-3.5 transition-transform duration-200 ease-paper {open ? 'rotate-180' : ''}"
		/>
	</div>

	{#snippet content()}
		<div
			class="text-sm border-hairline border-gray-50 dark:border-gray-850 rounded-xl my-1.5 p-2 w-full"
		>
			{#if status?.query}
				<a
