<script lang="ts">
	...
	interface Props {
		...
		onRegenerateWithModel?: Function;
		onClose?: Function;
	...
	let {
		...
		onRegenerateWithModel = (modelId: string, preserveToolContext?: boolean) => {},
		onClose = () => {},
		currentModelId = '',
		retryModelId = '',
		hasToolCalls = false,
	...
	let preserveContext = $state(true);

	// Filter to only show available models
	let availableModels = $derived($models.filter((m) => !(m?.info?.meta?.hidden ?? false)));
</script>
	...
	{#snippet content()}
		<div>
			<DropdownMenu.Content
				class="w-full max-w-[280px] rounded-2xl px-1 py-1 border-hairline border-gray-100 dark:border-gray-800 z-50 bg-white dark:bg-gray-850 dark:text-white shadow-lg transition"
				sideOffset={-2}
				side="bottom"
				align="start"
			...
			>
				{#if showModelSelector}
					<!-- Model Selector View -->
					<div
						class="flex items-center gap-2 px-3 py-2 border-b-hairline border-gray-100 dark:border-gray-800"
					>
						<button
					...
					{#if hasToolCalls}
						<div class="px-3 py-2 border-b-hairline border-gray-100 dark:border-gray-800">
				...
				{:else}
						...
						<div class="ml-2 self-center flex items-center">
							<button
								class="{inputValue !== ''
									...
									: 'text-white bg-gray-200 dark:text-gray-900 dark:bg-gray-700 disabled'} transition-colors duration-200 ease-paper rounded-full p-1 self-center"
								onclick={() => {
									onRegenerate(inputValue);
									show = false;
								}}
					...
					{#if hasToolCalls}
						<DropdownMenu.Item
							class="flex gap-2 items-center px-3 py-1.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl"
							onclick={() => {
								onRegenerateWithModel(retryModelId, true);
								show = false;
							}}
						>
							<svg
