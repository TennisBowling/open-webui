	...
	{#snippet content()}
		<div>
			<DropdownMenu.Content
				class="w-full max-w-70 rounded-2xl px-1 py-1  border-hairline border-gray-100  dark:border-gray-800 z-50 bg-white dark:bg-gray-850 dark:text-white shadow-lg max-h-72 overflow-y-auto overflow-x-hidden scrollbar-thin transition"
				sideOffset={4}
				alignOffset={-6}
				side="bottom"
				align="start"
			...
			>
				{#if tab === ''}
					<div in:fly={{ x: -20, duration: 150 }}>
						...
						>
							<DropdownMenu.Item
								class="flex gap-2 items-center px-3 py-1.5 max-md:py-2.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl {!fileUploadEnabled
									? 'opacity-50'
									: ''}"
								onclick={() => {
						...
						{#if $config?.features?.enable_video_input && addVideoHandler}
							<Tooltip
								content={!videoSupported
									? $i18n.t('Model(s) do not support video input')
									: !fileUploadEnabled
										? $i18n.t('You do not have permission to upload files.')
										: ''}
								className="w-full"
							>
								<DropdownMenu.Item
									class="flex gap-2 items-center px-3 py-1.5 max-md:py-2.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl {!videoSupported ||
									!fileUploadEnabled
										? 'opacity-50'
										: ''}"
									onclick={() => {
										if (videoSupported && fileUploadEnabled) {
											addVideoHandler();
										}
									}}
								>
									...
									<div class="line-clamp-1">{$i18n.t('Video')}</div>
								</DropdownMenu.Item>
							</Tooltip>
						{/if}
						...
						{#if fileUploadEnabled}
							{#if $config?.features?.enable_google_drive_integration}
								<DropdownMenu.Item
									class="flex gap-2 items-center px-3 py-1.5 max-md:py-2.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl"
									onclick={() => {
										uploadGoogleDriveHandler();
									}}
								>
							...
							{#if $config?.features?.enable_onedrive_integration && ($config?.features?.enable_onedrive_personal || $config?.features?.enable_onedrive_business)}
								<button
									class="flex gap-2 w-full items-center px-3 py-1.5 max-md:py-2.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl {!fileUploadEnabled
										? 'opacity-50'
										: ''}"
									onclick={() => {
				...
				{:else if tab === 'microsoft_onedrive'}
					<div in:fly={{ x: 20, duration: 150 }}>
						...
						{#if $config?.features?.enable_onedrive_personal}
							<DropdownMenu.Item
								class="flex gap-2 items-center px-3 py-1.5 max-md:py-2.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl text-left"
								onclick={() => {
