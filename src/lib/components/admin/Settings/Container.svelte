...
<form class="flex flex-col h-full justify-between text-sm" onsubmit={preventDefault(saveHandler)}>
			...
			<div class="mb-3">
				...
				<div class="mb-3 flex w-full justify-between items-center gap-4">
					<div>
						...
						<div class="text-xs text-gray-500 mt-0.5">
							{$i18n.t(
								'Only syncs files when the selected MCP external tool server is active for the message.'
							)}
						</div>
					</div>
				...
				<div class="mb-3">
					...
					<input
						...
						class="w-full mt-1 text-sm bg-transparent outline-hidden border-hairline border-gray-100 dark:border-gray-800 rounded-lg px-3 py-2"
						type="text"
				...
				<div class="mb-3">
					...
					<div class="text-xs text-gray-500 mt-1">
						{$i18n.t(
							'This is the ID from Admin Settings → External Tools. That MCP connection should send'
						)}
				...
				<div class="mb-3">
					...
					<textarea
						...
						class="w-full mt-1 text-sm bg-transparent outline-hidden border-hairline border-gray-100 dark:border-gray-800 rounded-lg px-3 py-2 min-h-36 resize-y"
						bind:value={containerConfig.CONTAINER_SYSTEM_PROMPT}
						placeholder={$i18n.t(
							'Instructions appended only when the container MCP server is active'
						)}></textarea>
					<div class="text-xs text-gray-500 mt-1">
						{$i18n.t(
							'This text is appended to the system prompt only for messages using the configured container MCP server. {{name}} also appends the current input/output file list automatically.',
							{ name: $WEBUI_NAME }
						)}
					</div>
				</div>
	...
	<div class="flex justify-end pt-3 text-sm font-medium">
		<button
			class="px-3.5 py-1.5 text-sm font-medium bg-book-cloth hover:bg-kraft text-white dark:bg-book-cloth dark:text-white dark:hover:bg-kraft transition-colors duration-200 ease-paper rounded-full flex items-center gap-2 {saving
